...
mod tests {
    ...
    fn test_take() {
        ...
        // Airdrop some SOL to the payer keypair
        program
            .airdrop(&payer.pubkey(), 20 * LAMPORTS_PER_SOL)
            .unwrap();
        ...
        // Send the transaction and capture the result
        let _tx = program.send_transaction(transaction).unwrap();
        ...
        let escrow_opt = program.get_account(&escrow);
        assert!(escrow_opt.is_none() || escrow_opt.unwrap().lamports == 0);

        // check if balances moved correctly
        ...
        assert_eq!(taker_ata_a_data.amount, 10); // received deposit amount
        ...
        assert_eq!(maker_ata_b_data.amount, 10); // received the 'receive' amount
    }
    ...
    fn test_refund() {
        ...
        let response = program.send_transaction(transaction).unwrap();

        // Log transaction details
        msg!("\n\nMake transaction sucessfull");
        msg!("CUs Consumed: {}", response.compute_units_consumed);
        msg!("Tx Signature: {}", response.signature);
    }
    ...
    fn test_take_after_five_days() {
        ...
        let mut clock: Clock = program.get_sysvar(); // or program.get_sysvar() if using recent litesvm

        let five_days= 5 * 24 * 60 * 60;
        clock.unix_timestamp += five_days + 1; // +1s to be clearly past
